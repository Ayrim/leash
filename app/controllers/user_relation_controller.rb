class UserRelationController < ApplicationController
  before_action :require_login

    def index
        respond_to do |format|
            format.html { redirect_to :controller => "user_relation" , :action => "show" }
        end
    end
  	...
  	def destroy
    		...
    		if(params.has_key?(:id))
    			if(params[:id] != 'update_unreadmessages')
                	@connId = params[:id]
      			   	@delete_connection = UserRelation.where('(user_1_id = ? AND user_2_id = ?) OR (user_1_id = ? AND user_2_id = ?)', current_user.id, params[:id], params[:id], current_user.id).first

      			   	if @delete_connection.is_pending
      			   		#send mail to notify the user that sent the invitation it is being rejected
						rejectedConnection = User.find_by(:id => params[:id])
						if(!rejectedConnection.nil?)
                        	rejectedConnection.send_rejection_mail(current_user)
                    	end
      			   	end
        			...
        			if @delete_connection.destroy
                    	...
                    	set_connections()
          				respond_to do |format|
        			    	format.html { redirect_to :controller => "user_relation" , :action => "show" }
        			  	 	format.js { render 'user_relation/show_update_connections.js.erb' }
        			    end
        			else
        			    @showModal = true;
        			    flash.now.alert = "Something went wrong when attempting to remove this connection. Please, try again later."
                    	render 'user_relation/show_update_connections.js.erb'
        			end
      			end
    		else
      			@showModal = true;
  	...
  	def accept_invitation
    		...
    		if(params.has_key?(:id))
      			if(params[:id] != 'update_unreadmessages')
                @connId = params[:id]
          			update_connection = UserRelation.where('(user_1_id = ? AND user_2_id = ?) OR (user_1_id = ? AND user_2_id = ?)', current_user.id, params[:id], params[:id], current_user.id).first
          			begin
                    ActiveRecord::Base.transaction do
                      	if(!update_connection.nil?)
                            ...
                            render 'user_relation/show_acceptance_alert.js.erb'
                				end
                    end
